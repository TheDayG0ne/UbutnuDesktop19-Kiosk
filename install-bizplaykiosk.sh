...
Type=Application
Name=chromium
Exec=chromium-browser --password-store=basic --no-first-run  --no-default-browser-check --disable --disable-translate --disable-session-crashed-bubble --disable-restore-session-state --disable-infobars --disable-suggestions-service --disable-save-password-bubble --disable-features=InfiniteSessionRestore --start-maximized --kiosk "http://play.playr.biz"
